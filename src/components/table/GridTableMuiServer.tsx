...
import { DataGrid, GridRowsProp, GridColDef, GridRowId, GridToolbar, GridToolbarContainer, GridToolbarColumnsButton, GridToolbarFilterButton, GridToolbarDensitySelector, GridToolbarExport, GridActionsCellItem, GridRowParams, GridActionsCellItemProps, GridFilterModel } from '@mui/x-data-grid';
...
type serverSide = {
  isActive: boolean
  filters?: (val: GridFilterModel) => void
  pagination: (val: { pageSize: number, page: number, }) => void
}
...
const GridTableMuiServer = (props: Props) => {
  ...
  React.useEffect(() => {
    if (props.serverSide?.isActive) {
      props.serverSide.pagination(paginationModel)
    }
  }, [paginationModel])
  ...
  const handleRefresh = () => {
    if (typeof props.toolbarRefresh?.handleClick === 'function' && props.toolbarRefresh?.handleClick !== undefined) {
      setIsLoading(true)
  ...
  const customToolbar = React.useCallback(() => {
    return (
      <GridToolbarContainer>
        ...
        {props.toolbarExport?.isActive && (<GridToolbarExport />)}
      </GridToolbarContainer>
    );
  }, [props])

  const onFilterChange = React.useCallback((filterModel: GridFilterModel) => {
    if (props.serverSide?.isActive && props.serverSide?.filters !== undefined) {
      props.serverSide.filters(filterModel)
    }
  }, []);
...
export default GridTableMuiServer
