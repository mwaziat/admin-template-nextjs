...
export const ViewDataRoles = async (): Promise<{ status: boolean, error: any, data: any[] | undefined }> => {
...
export const DeleteManyData = async (data: any): Promise<{ status: boolean, error: any, data: any }> => {
	return await DELETE(`${process.env.NEXT_PUBLIC_API_SERVER!}/users-management/roles/delete-many`, data)
	.then((res) => {
			if(res.status !== 0){
				return { status: true, error: null, data: res };
			} else {
